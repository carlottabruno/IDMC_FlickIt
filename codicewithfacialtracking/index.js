...
function ricalcolaLayout() {
  ...
  bottoneMano.w = bW; bottoneMano.h = bH;
  bottoneViso.w = bW; bottoneViso.h = bH;
  bottoneMano.x = width / 2 - bW - sx(20);
  bottoneMano.y = height / 2 - bH / 2;
  bottoneViso.x = width / 2 + sx(20);
  bottoneViso.y = height / 2 - bH / 2;
  ...
  btnMenuPausa.x = width / 2 - pW / 2;
  btnMenuPausa.y = height / 2 + sy(110);
}
    ...
    .then(function(devices) {
      ...
      devices.forEach(function(device) {
        if (device.kind === 'videoinput') {
          if (!device.label.includes('FaceTime') && !device.label.includes('Integrated')) {
...
function draw() {
  ...
  if (schema === 4) {
    ...
    if ((modalitaGioco === "mano" || modalitaGioco === "viso") && video && video.loadedmetadata) {
      ...
      push();
      translate(width - vW - sx(10), sy(10));
      scale(-1, 1);
  ...
  if (schema === 3) {
    ...
    fill(255); noStroke(); textAlign(CENTER, CENTER);
    textSize(ss(80));
    text("HAI VINTO!", width / 2, height / 2 - sy(100));
    textSize(ss(40));
  ...
  for (let i = 0; i < hands.length; i++) {
    ...
    if (hand.keypoints) {
      for (let j = 0; j < hand.keypoints.length; j++) {
        ...
        fill(0, 255, 0); noStroke(); circle(kx, ky, ss(10));
      }
    }
  }
...
function inizializzaCarte(lv) {
  ...
  let marginBottom = sy(40);
  let areaW = width  - marginLeft - marginRight;
  let areaH = height - marginTop  - marginBottom;
  ...
  function grigliaPos(nCols, nRows, colIdx, rowIdx) {
    ...
    return {
      x: startX + colIdx * (cW + gap),
      y: startY + rowIdx * (cH + gap)
    };
  }
